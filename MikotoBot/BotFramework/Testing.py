...
class TestingClient:
		...
		for tests in list(self.testList.keys()):
			...
			for test in list(self.testList[tests]["Tests"].keys()):
				...
				for result in currentTest["Tests"][test][1]:
					...
					if re.fullmatch(result, self.comResult):
	...
	def _failedTest(self, testGroup, test, result, expected):
		...
		testResult=("Failed Test\n"
			"	Result.........: {}\n"
			"	Expected Result: {}".format(repr(result), repr(expected)))
...
class SendMessageChannel:
	...
	async def send(self, message):
		...
		messageObj = MessageObject(message, testID=0)
		messageObj.author.name = "MikotoBot"
		messageObj.author.nick = "Mikoto"
		self.test.bot.logging.messageLog(messageObj)
